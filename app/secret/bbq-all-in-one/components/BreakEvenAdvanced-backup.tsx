...
export default function BreakEvenAdvancedPage() {
  ...
  useEffect(() => {
    setMounted(true)
    const saved = localStorage.getItem("breakEvenAdvancedData")
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("breakEvenAdvancedData", JSON.stringify(data))
    ...
    <div className="container mx-auto py-8 px-4 max-w-7xl">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold mb-2">고급 손익분기 계산기</h1>
            <p className="text-muted-foreground">메뉴별 상세 분석 및 시나리오 시뮬레이션</p>
          </div>
          <Link href="/">
        ...
        <Alert className="mb-6">
          ...
          <AlertDescription className="text-sm space-y-1">
            <p>• 각 메뉴의 판매가, 원가, 판매 비중을 입력하세요</p>
            <p>• 판매 비중은 전체 매출에서 해당 메뉴가 차지하는 비율입니다</p>
            <p>• 월 고정비(임대료, 인건비 등)와 목표 이익을 입력하면 필요한 테이블 수가 자동 계산됩니다</p>
          </AlertDescription>
        </Alert>
      </div>
