...
const RadarChart3D: React.FC<{ data: { [key: string]: number }, factorNames: { [key: string]: string } | ((key: string) => string) }> = ({ data, factorNames }) => {
  ...
  return (
    <div className="relative w-full h-96 flex items-center justify-center bg-gradient-to-br from-indigo-50 to-blue-50 rounded-xl overflow-hidden">
            ...
            <g key={`label-${factor}`}>
              {words.map((word, idx) => (
                <text
                  ...
                  className="text-[8px] font-bold fill-indigo-700"
                >
                  {word}
                </text>
              ))}
