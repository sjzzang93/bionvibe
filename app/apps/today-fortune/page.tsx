    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 py-12 px-4">
        ...
        <div className="max-w-6xl mx-auto mb-16 text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-black mb-6 bg-gradient-to-r from-yellow-200 via-pink-200 to-purple-200 bg-clip-text text-transparent">
            오늘의 운세
          </h1>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {zodiacSigns.map((zodiac, index) => (
              <button
                ...
                className={`group relative bg-white/10 backdrop-blur-xl hover:bg-white/20 rounded sm:rounded-lg md:rounded-2xl p-6 transition-all duration-300 hover:scale-105 hover:-translate-y-2 border border-white/20 overflow-hidden ${
                  selectedZodiac === zodiac.name ? 'ring-4 ring-yellow-400 bg-white/20 scale-105' : ''
                }`}
                style={{
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="text-6xl mb-0.5 sm:mb-1.5 md:mb-2 transition-transform duration-300 group-hover:scale-125">{zodiac.emoji}</div>
                  <div className="text-white font-bold text-[10px] sm:text-xs md:text-sm mb-1">{zodiac.name}</div>
                  <div className="text-white/60 text-xs">{zodiac.date}</div>
                </div>
        ...
        {isAnimating && (
          <div className="max-w-2xl mx-auto text-center py-20">
            <div className="inline-block">
              ...
              <p className="text-white text-xl font-light animate-pulse">운세를 분석하고 있습니다...</p>
            </div>
          </div>
        )}
        ...
        {/* 결과 */}
        {result && !isAnimating && (
          <div className={`max-w-4xl mx-auto space-y-6 transition-all duration-700 ${showResult ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            {/* Main Card */}
            <div className="bg-white/10 backdrop-blur-2xl rounded-3xl shadow-2xl p-8 md:p-12 border border-white/20 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-4xl md:text-5xl font-black text-white mb-0.5 sm:mb-1.5 md:mb-2">{result.zodiac}</h2>
                  <div className="inline-block px-6 py-2 bg-white/20 rounded-full text-white/80 text-sm">
                    {new Date().toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'long' })}
                  </div>
                </div>

                {/* 종합 운세 */}
                <div className="bg-white/10 backdrop-blur-xl rounded sm:rounded-lg md:rounded-2xl p-8 mb-8 border border-white/20">
                ...
                {/* 메시지 */}
                <div className="bg-gradient-to-r from-blue-500/20 to-purple-500/20 backdrop-blur-xl rounded sm:rounded-lg md:rounded-2xl p-8 mb-8 border border-white/30 relative overflow-hidden">
                  <div className="absolute top-0 right-0 text-9xl text-white/5 leading-none">💬</div>
                  <h3 className="text-2xl font-bold text-white mb-4 relative z-10">💬 오늘의 메시지</h3>
                  <p className="text-white/90 leading-relaxed text-lg md:text-xl font-light relative z-10">{result.message}</p>
                </div>

                {/* 행운 아이템 */}
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-6 mb-8">
                  <div className="bg-gradient-to-br from-pink-500/20 to-rose-500/20 backdrop-blur-xl rounded sm:rounded-lg md:rounded-2xl p-6 border border-white/20 hover:scale-105 transition-transform duration-300">
                    <h3 className="text-xl font-bold text-white mb-0.5 sm:mb-1.5 md:mb-2 flex items-center gap-2">
                      <span className="text-3xl">🎨</span> 행운의 색상
                    </h3>
                    <p className="text-4xl font-black bg-gradient-to-r from-yellow-200 to-pink-200 bg-clip-text text-transparent">{result.luckyColor}</p>
                  </div>
                  <div className="bg-gradient-to-br from-yellow-500/20 to-orange-500/20 backdrop-blur-xl rounded sm:rounded-lg md:rounded-2xl p-6 border border-white/20 hover:scale-105 transition-transform duration-300">
                    <h3 className="text-xl font-bold text-white mb-0.5 sm:mb-1.5 md:mb-2 flex items-center gap-2">
                      <span className="text-3xl">🔢</span> 행운의 숫자
                    </h3>
                    <p className="text-4xl font-black bg-gradient-to-r from-yellow-200 to-orange-200 bg-clip-text text-transparent">{result.luckyNumber}</p>
                  </div>
                </div>

                {/* 상세 조언 */}
                <div className="bg-gradient-to-br from-purple-500/20 to-indigo-500/20 backdrop-blur-xl rounded sm:rounded-lg md:rounded-2xl p-8 border border-white/30">
                  <h3 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
                    <span className="text-3xl">💡</span> 시간대별 조언
                  </h3>
                  <div className="space-y-4">
                    {result.detailedAdvice.map((advice, index) => (
                      <div key={index} className="flex gap-4 items-start group hover:translate-x-2 transition-transform duration-300">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-yellow-400 to-orange-400 flex items-center justify-center flex-shrink-0 font-bold text-white shadow-lg">
                          {index + 1}
                        </div>
                        <p className="text-white/90 leading-relaxed pt-1">{advice}</p>
                      </div>
                    ))}
                  </div>
                </div>
