...
-- 6단계: updated_at 자동 업데이트 함수 (이미 있으면 재사용)
...
-- 숨김 앱 확인
SELECT id, name, hidden FROM apps WHERE hidden = true ORDER BY name;

-- 전체 앱 목록
SELECT id, name, category_id, image FROM apps ORDER BY name;
