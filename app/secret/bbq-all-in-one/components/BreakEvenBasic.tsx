...
export default function BreakEvenBasic() {
  const [mounted, setMounted] = useState(false)

  // 입력값
  const [asp, setAsp] = useState(56000) // 평균 테이블 단가
  ...
  useEffect(() => {
    setMounted(true)
    const saved = localStorage.getItem("breakEvenBasicData")
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("breakEvenBasicData", JSON.stringify(data))
  ...
  const result = useMemo(() => {
    ...
    const cmr = 1 - varRateDecimal

    const bepRevenue = cmr > 0 ? fixedDay / cmr : NaN
    const bepTables = asp > 0 ? bepRevenue / asp : NaN

    const needRevenue = cmr > 0 ? (fixedDay + targetProfitDay) / cmr : NaN
    const needTables = asp > 0 ? needRevenue / asp : NaN

    const currentRevenue = asp * tablesHint
    const currentProfit = currentRevenue * cmr - fixedDay
  ...
  if (!mounted) {
    return <div className="flex items-center justify-center py-8">로딩 중...</div>
  }
