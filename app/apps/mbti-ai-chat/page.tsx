...
// MBTI 페르소나 데이터 (간소화 버전)
const MBTI_PERSONAS = {
  'ISTJ': { name: '원칙주의 실무가', emoji: '📋', greeting: '안녕하세요. 오늘 계획부터 차근차근 정리해볼까요?' },
  'ISFJ': { name: '헌신적 보호자', emoji: '🤗', greeting: '안녕하세요! 편하게 이야기 나눠요. 어떤 도움이 필요하세요?' },
  'INFJ': { name: '통찰력 있는 조언자', emoji: '🔮', greeting: '반가워요. 당신의 내면에 대해 이야기해볼까요?' },
  'INTJ': { name: '전략적 설계자', emoji: '🎯', greeting: '안녕하세요. 목표를 효율적으로 달성하는 방법을 찾아볼까요?' },
  'ISTP': { name: '논리적 실험가', emoji: '🔧', greeting: '안녕. 뭐 재밌는 문제 있어?' },
  'ISFP': { name: '예술적 모험가', emoji: '🎨', greeting: '안녕! 오늘은 어떤 감성으로 가득 찬 날인가요?' },
  'INFP': { name: '이상주의 몽상가', emoji: '🌈', greeting: '안녕하세요... 오늘 어떤 생각에 잠겨 계신가요?' },
  'INTP': { name: '논리적 사색가', emoji: '🧠', greeting: '흥미로운 주제가 있나요? 깊이 파고들어볼까요?' },
  'ESTP': { name: '행동파 도전가', emoji: '⚡', greeting: '야! 뭐 재밌는 거 없어? 바로 해보자!' },
  'ESFP': { name: '자유로운 연예인', emoji: '🎉', greeting: '헤이! 오늘 완전 신나는 일 있었어? 나한테 얘기해줘!' },
  'ENFP': { name: '열정적 활동가', emoji: '✨', greeting: '와! 오늘 만나서 너무 좋아요! 뭔가 재밌는 이야기 해요!' },
  'ENTP': { name: '발명가형 논객', emoji: '💡', greeting: '오! 뭔가 논쟁거리가 있나? 재밌는 토론 해보자!' },
  'ESTJ': { name: '엄격한 관리자', emoji: '👔', greeting: '안녕하십니까. 오늘의 업무부터 체계적으로 정리하죠.' },
  'ESFJ': { name: '사교적 외교관', emoji: '💝', greeting: '안녕하세요! 오늘 하루 어떠셨어요? 편하게 얘기해요!' },
  'ENFJ': { name: '카리스마 지도자', emoji: '🌟', greeting: '반갑습니다! 당신의 잠재력을 함께 발견해볼까요?' },
  'ENTJ': { name: '대담한 통솔자', emoji: '👑', greeting: '안녕하세요. 목표를 설정하고 빠르게 실행해봅시다.' },
};
...
export default function MBTIAIChatPage() {
          ...
          <section className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
            {Object.entries(MBTI_PERSONAS).map(([mbti, persona]) => (
              <button
                ...
                className="group relative bg-white/10 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-6 border-2 border-white/20 hover:border-white/40 hover:bg-white/20 transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-purple-500/50"
              >
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-purple-900 via-indigo-900 to-blue-900 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 py-6 sm:py-8 h-screen flex flex-col">
        {/* 헤더 */}
        <header className="bg-white/10 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-6 border border-white/20 mb-4 sm:mb-6 shadow-2xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="text-4xl sm:text-5xl md:text-6xl">{persona.emoji}</div>
              <div>
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-white">
                  {selectedMBTI}
                </h1>
                <p className="text-sm sm:text-base text-white/70">{persona.name}</p>
              </div>
            </div>
            <button
              type="button"
              onClick={handleReset}
              className="px-4 sm:px-6 py-2 sm:py-3 bg-white/20 hover:bg-white/30 rounded-xl text-white font-bold transition-all duration-300 text-sm sm:text-base"
            >
              🔄 다시 선택
            </button>
          </div>
        ...
        {/* 메시지 영역 */}
        <div className="flex-1 bg-white/5 backdrop-blur-xl rounded-2xl sm:rounded-3xl border border-white/20 p-4 sm:p-6 mb-4 overflow-y-auto shadow-2xl">
          <div className="space-y-4">
            {messages.map((msg, idx) => (
              ...
              >
                <div
                  className={`max-w-[80%] sm:max-w-[70%] px-4 sm:px-6 py-3 sm:py-4 rounded-2xl ${
                    msg.role === 'user'
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-white/20 px-6 py-4 rounded-2xl border border-white/30">
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-6 border border-white/20 shadow-2xl">
          <div className="flex gap-3">
            <textarea
              ...
              className="flex-1 bg-white/20 text-white placeholder-white/50 rounded-xl px-4 sm:px-6 py-3 sm:py-4 border border-white/30 focus:border-white/50 focus:outline-none resize-none text-sm sm:text-base"
              rows={2}
              disabled={isLoading}
            />
            <button
              ...
              className="px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 disabled:from-gray-500 disabled:to-gray-600 disabled:cursor-not-allowed rounded-xl text-white font-bold transition-all duration-300 shadow-lg hover:shadow-2xl hover:scale-105 text-sm sm:text-base"
            >
              전송
            </button>
          </div>
