...
export default function MbtiZodiacCompatibilityPage() {
  const [selectedMbti, setSelectedMbti] = useState(MBTI_OPTIONS[0]?.type ?? 'ENFP');
  const [selectedZodiac, setSelectedZodiac] = useState(ZODIAC_OPTIONS[0]?.id ?? 'rat');

  const mbtiProfile = useMemo(
    () => MBTI_OPTIONS.find((item) => item.type === selectedMbti) ?? MBTI_OPTIONS[0],
    [selectedMbti],
  );
  const zodiacProfile = useMemo(
    () => ZODIAC_OPTIONS.find((item) => item.id === selectedZodiac) ?? ZODIAC_OPTIONS[0],
    [selectedZodiac],
  );

  const compatibility = useMemo(
    () => getCompatibility(selectedMbti, selectedZodiac),
    [selectedMbti, selectedZodiac],
  );
  const recommendedZodiacs = useMemo(
    () => getRecommendedZodiacs(selectedMbti),
    [selectedMbti],
  );
          ...
          <section className="relative overflow-hidden rounded-[36px] border border-white/40 bg-gradient-to-br from-[#1c1a3a] via-[#221c4e] to-[#0f1538] text-white shadow-2xl">
            ...
            <div className="relative z-10 px-6 py-12 sm:px-12 sm:py-16">
              ...
              <p className="mt-4 max-w-2xl text-base text-white/75 sm:text-lg">
                16가지 MBTI 핵심 성향과 12띠 페르소나, 오행 흐름을 결합해 하이브리드 궁합을
                제공합니다. 데이터 기반으로 제작되어 Google AdSense 정책에 적합한 안전한 콘텐츠를
                지향합니다.
              </p>
          ...
          <section className="mt-12 space-y-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm text-gray-600 dark:text-gray-400 sm:text-base">
                  분석형 · 외교형 · 수호형 · 탐험형 그룹을 모두 아우르는 카드입니다. 클릭하면 카드가
                  3D로 반응하며, 선택한 유형을 기준으로 최적의 띠 추천이 업데이트됩니다.
                </p>
              </div>
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      ...
                      isSelected
                        ? [
                            'border-transparent text-white shadow-[0_25px_60px_-15px_rgba(99,102,241,0.55)]',
                            'bg-gradient-to-br',
                            option.accent,
                            'hover:-translate-y-1.5',
                          ]
                        : [
                            'border-gray-200/80 bg-white/90 text-gray-900 shadow-sm',
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="mt-3 text-2xl font-black">{option.type}</div>
                        <p
                          className={clsx(
                            'mt-1 text-sm',
                      ...
                      <div
                        className={clsx(
                          'flex h-12 w-12 items-center justify-center rounded-2xl border text-lg font-semibold backdrop-blur',
                          isSelected
                    ...
                    <div
                      className={clsx(
                        'mt-6 flex flex-wrap items-center gap-2 text-xs font-medium',
            ...
            <div className="rounded-2xl border border-gray-200 bg-white/90 p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900/80">
              <div className="flex flex-wrap items-center gap-2 text-sm text-gray-700 dark:text-gray-200">
                <span className="font-semibold">추천 띠</span>
                {recommendedZodiacs.map((zodiac) => {
                  ...
                  return (
                    <button
                      ...
                      className={clsx(
                        'inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm transition-all',
          ...
          <section className="mt-16 space-y-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm text-gray-600 dark:text-gray-400 sm:text-base">
                  각 띠는 오행과 음양, 연령대별 대표 특징을 결합한 페르소나입니다. 선택하면 카드
                  하이라이트가 3D로 움직이며 맞춤 분석으로 이어집니다.
                </p>
              </div>
              <div className="rounded-full border border-slate-200 bg-slate-100 px-4 py-1 text-xs font-semibold text-slate-600 shadow-sm dark:border-slate-600/60 dark:bg-slate-800/70 dark:text-slate-300">
                현재 선택: {zodiacProfile.name} · {zodiacProfile.elementLabel} · {zodiacProfile.yinYang}
              </div>
            </div>
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      ...
                      isSelected
                        ? [
                            'border-transparent text-white shadow-[0_22px_55px_-20px_rgba(94,129,244,0.55)]',
                            'bg-gradient-to-br',
                            ELEMENT_CHIP_GRADIENT[zodiac.element],
                            'hover:-translate-y-1.5',
                          ]
                    ...
                    <div
                      className={clsx(
                        ...
                        isSelected
                          ...
                          : 'border-gray-200 bg-gray-100 text-gray-600 dark:border-gray-600 dark:bg-gray-800/80 dark:text-gray-200',
                      )}
                    >
                      <span>{zodiac.elementLabel}</span>
                      <span>·</span>
                      <span>{zodiac.yinYang}</span>
                    </div>
                    <p
                      className={clsx(
                  ...
                  <div className="flex flex-col gap-6">
                    <div className="space-y-4 rounded-3xl border border-white/15 bg-black/30 p-6 backdrop-blur">
                      <div className="flex flex-wrap items-center gap-3 text-sm font-semibold uppercase tracking-[0.3em] text-white/60">
                        ...
                        <span>{mbtiProfile.type}</span>
                        <span>×</span>
                        <span>{zodiacProfile.name}</span>
                      </div>
